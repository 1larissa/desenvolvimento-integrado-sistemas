...
class Server:
    def __init__(self):
        ...
        # os numeros magicos
        self.LIMITE_MEMORIA_MB = 85.0
        self.LIMITE_CPU_PERCENT = 85.0
        self.threads_ativas = 0
        self.MAX_THREADS = 4

    def iniciarServidor(self):
        # inicializa o servidor em outra thread
        print("Servidor Python Inicializado!")
        # threading.Thread(target=self._gerenciar_fila, daemon=True).start()

        # # faz coisas para ligar as comunicacoes/requests
        # #...
        
        # # Inicia uma thread separada para gerenciar a fila
        # threading.Thread(target=self._gerenciar_fila, daemon=True).start()
    ...
    def adicionarSolicitacao(self, nova_solicitacao: Solicitacao):
        ...
        print(f"{nova_solicitacao.id} adicionada à fila.")
    ...
    def _gerenciar_fila(self):
        # Monitora os recursos e gerencia a fila de solicitacoes, em uma thread separada
        # Esta função deve rodar em uma thread separada iniciada pelo iniciarServidor().
    ...
    def _processar_imagem(self, solicitacao: Solicitacao):
        ...
        try:
            ...
            func = cgnr if solicitacao.algoritmo.upper() == 'CGNR' else cgne
    ...
    def run_in_Python(self):
        try:
            ...
            df_H = pd.read_csv(ARQUIVO_MATRIZ_H, sep=SEP_CHAR, header=None)
            df_g = pd.read_csv(ARQUIVO_VETOR_G, sep=SEP_CHAR, header=None)
